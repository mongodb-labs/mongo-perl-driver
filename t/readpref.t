...
subtest "three-node replica set" => sub {
    ...
    # commands
    {
        ...
        my $admin = $rsconn->get_database('admin');
        my $cmd = $admin->get_collection('$cmd');

        my $cmd_conn = MongoDB::Collection::_select_cursor_client($rsconn, 'admin.$cmd',
            {renameCollection => 'foo.bar', to => 'foo.foofoo'});
        is($cmd_conn, $rsconn->_master, 'renameCollection runs on primary');

        $cmd_conn = MongoDB::Collection::_select_cursor_client($rsconn, 'admin.$cmd',
