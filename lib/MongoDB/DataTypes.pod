...
number as a 64-bit integer, use bigint (aka L<Math::BigInt>):
...
One technique to eliminate the string representation and store a numeric
...
codes.  If we wanted to be sure it is saved as a string, bless the string
as a C<MongoDB::BSON::String> type:
...
The L<DateTime>, L<Time::Moment> or L<DateTime::Tiny> packages can be used
...
using the L<Time::HiRes time/Time::HiRes#time> function to get epoch
seconds as a floating-point value.

The L<Time::Moment> module is substantially faster than L<DateTime> and
might be a convenient alternative over using integers or floating point
numbers and manually inflating to an object before use.  Consider
benchmarking using C<dt_type> set to L<Time::Moment> against using numbers
...
the C<try_compile> method, though due to PCRE differences, this could fail
...
"OID" stands for "Object ID", and is a unique id to identify documents.
They are 12 bytes which are guaranteed to be unique.  Their string form is
...
L<MongoDB::BSON::Binary> object stringify to the underlying data to make it
easier to work with.
