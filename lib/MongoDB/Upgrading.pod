...
under Perl 5.8 were so broken that driver tests were regularly failing.
...
For update details on types, see L<MongoDB::DataTypes>.  The following
...
The C<dt_type> configuration argument has been removed from
L<MongoDB::MongoClient> and the default date type of the L<BSON> library is
now L<BSON::Time>, which is extremly lightweight and provides convenience
methods to convert to various popular time classes.  It also works will
with L<Time::HiRes> for recording datetimes with millisecond precision.

Code that relied on date types being L<DateTime> objects will need to
convert via the C<as_datetime> method.
...
clear how it treated them, plus the treatment could vary slightly by Perl
version.  The heuristics are now standardized as follows:
...
To simplify use of type wrappers (and avoid unintentionally using a
...
=head2 run_command requires ordered document
...
The MONGODB-CR authentication mechanism was deprecated in MongoDB serve 3.6
and removed in MongoDB 4.0.  The Perl driver is deprecating MONGODB-CR, but
will not remove it until it no longer supports older servers.
