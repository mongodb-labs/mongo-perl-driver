...
* consistency (intra-driver) – many parts of the v0 API were inconsistent,
  ...
  exception types.
* consistency (inter-driver) — "next-generation" MongoDB drivers across
  all languages are converging on common APIs and common behaviors; this
...
* portability – the v0 driver had a large dependency tree and required a
  ...
  and uses widely-used CPAN modules in place of custom C code where
  possible.
