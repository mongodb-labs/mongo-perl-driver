...
=head3 Regular expressions always decode to C<MongoDB::BSON::Regexp> objects

Previously, BSON regular expressions decoded to C<qr{}> references by
default and the C<inflate_regexps> option was available to decode
instead to L<MongoDB::BSON::Regexps>.
...
=head3 Some C<MongoDB::MongoClient> options moved to C<MongoDB::BSON>

The C<inflate_dbrefs> option has been deprecated and replaced with a
C<dbref_callback> option in L<MongoDB::BSON>.  By default, the
C<MongoDB::MongoClient> will create a L<MongoDB::BSON> codec that
will construct L<MongoDB::DBRef> objects.

The C<dt_type> option now only takes effect if and when the C<MongoDB::MongoClient>
constructs the L<MongoDB::BSON> codec.  It has been changed to a read-only
attribute so that any code that relied on changing C<dt_type> after
constructing a C<MongoDB::MongoClient> object will fail.

=head3 Introduction of MongoDB::BSON::DateTime
