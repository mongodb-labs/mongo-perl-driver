...
Pparallel scan "cursors" are now L<QueryResult> objects, with the same
...
=head2 C<$MongoDB::BSON::use_boolean> removed
...
boolean deserialization without relying on global variables.
...
=head3 MongoDB::Collection::get_collection

This method implied that collections could be contained inside collection.
This doesn't actually happen so it's confusing to have a Collection be a
factory for collections.  Users who want nested namespaces will be explicit
and create them off Database objects instead.
