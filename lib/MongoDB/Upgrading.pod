...
SSL configuration is now possible via the L<MongoDB::MongoClient/ssl>
attribute.

Authentication configuration is described in
L<MongoDB::MongoClient/AUTHENTICATION>.
...
"handshake" with any given server based on
L<MongoDB::MongoClient/auth_mechanism>.

The C<MongoDB::MongoClient::authenticate> method is deprecated and the
...
=head3 auto_reconnect, find_master

These attributes no longer have any effect.  The driver always attempts to
reconnect on demand and to find an appropriate server.  See
L<MongoDB::MongoClient/SERVER SELECTION> for details on how reconnections
are handled.
...
L<MongoDB::MongoClient/auth_mechanism> and
L<MongoDB::MongoClient/auth_mechanism_properties> attributes.  These are
