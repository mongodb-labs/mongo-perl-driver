...
MongoDB 1.3+ supports a "find, modify, and return" command.  This command can be used to atomically modify a document (at most one) and return it.
Note that the document returned will not include the modifications made on the update.
The command includes sort option which is useful when storing queue-like data.
...
sub find_and_modify {
    my ($self,$options) = @_;
    confess 'expected hash reference for $options' unless ref $options eq 'HASH';
    confess 'At least one of the update or remove parameters is required' unless exists $options->{update} || exists $options->{remove};
    
    my $cmd = new Tie::IxHash(findandmodify => $self->name);
