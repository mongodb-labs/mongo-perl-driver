...
This is called an 'upsert' (as it is a combination of an update and an insert).  
For example, the same code could be used for creating and updating a log 
document:
...
For example, to create a non-unique index on 'x':
    ...
    $collection->ensure_index({'x' => 'ascending'})

To create a unique index on 'y':

    $collection->ensure_index({'y' => 'ascending', 'z' => 'descending'}, 1);

For multi-key indexes, keep in mind that you should use L<Tie::IxHash> to 
